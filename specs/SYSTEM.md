...
schematics, a custom PCB, and 74-series address-decode logic) plus a cycle-honest **C++
...
```
   ...
   Codegen (codegen/): 65C02 assembler + headless harness + platform-ref
                         ...
                         ▼
   Hardware (kicad/, logisim/, diylayout/): schematics, PCB, 74-series decode
```

- **Stack:** C++17 VM core → Emscripten/WebAssembly browser build; Node.js ESM tooling
  (65C02 assembler + headless emulator harness); 6502 assembly ROM/software; KiCad +
  74-series logic hardware. `22v10/` contains intermediate designs, not the final build.
...
| Hardware | [`HARDWARE.md`](./HARDWARE.md) | KiCad schematics/PCB, 74-series address decode, logisim/diylayout models; historical GAL experiments |
...
- **The memory map is a shared contract.** `emulator/Badger6502VMLib/vm.h` (the `MM_*` map
  + soft switches + ROM entry points) is mirrored by the hardware decode schematic, the web bridge,
