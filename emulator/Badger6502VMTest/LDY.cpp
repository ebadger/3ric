#include "pch.h"
#include "CppUnitTest.h"
#include "vm.h"

using namespace Microsoft::VisualStudio::CppUnitTestFramework;

namespace Badger6502VMTest
{
	TEST_CLASS(LDY)
	{
	public:

#pragma region LDY
		/*
			case LDY_IMM:
			case LDY_ABS:
			case LDY_ABS_X:
			case LDY_ZP:
			case LDY_ZP_X:
		*/
		TEST_METHOD(TEST_LDY_IMM)
		{
			uint32_t cycles = 0;
			VM vm(true);
			CPU* cpu = vm.GetCPU();

			vm.WriteData(0xFFFC, 0x00);
			vm.WriteData(0xFFFD, 0x10);

			// load 0x69 into the Y register
			vm.WriteData(0x1000, LDY_IMM);
			vm.WriteData(0x1001, 0x69);

			cpu->Reset();

			cycles = cpu->Step();

			Assert::IsTrue(cpu->Y == 0x69);
			Assert::IsTrue(cycles == 2);
		}

		TEST_METHOD(TEST_LDY_ABS)
		{
			uint32_t cycles = 0;
			VM vm(true);
			CPU* cpu = vm.GetCPU();

			vm.WriteData(0xFFFC, 0x00);
			vm.WriteData(0xFFFD, 0x10);

			vm.WriteData(0x2000, 0x56);

			vm.WriteData(0x1000, LDY_ABS);
			vm.WriteData(0x1001, 0x00);
			vm.WriteData(0x1002, 0x20);

			cpu->Reset();

			cycles = cpu->Step();

			Assert::IsTrue(cpu->Y == 0x56);
			Assert::IsTrue(cycles == 4);
		}

		TEST_METHOD(TEST_LDY_ABS_X)
		{
			uint32_t cycles = 0;
			VM vm(true);
			CPU* cpu = vm.GetCPU();

			vm.WriteData(0xFFFC, 0x00);
			vm.WriteData(0xFFFD, 0x10);

			vm.WriteData(0x2022, 0x53);

			vm.WriteData(0x1000, LDX_IMM);
			vm.WriteData(0x1001, 0x22);

			vm.WriteData(0x1002, LDY_ABS_X);
			vm.WriteData(0x1003, 0x00);
			vm.WriteData(0x1004, 0x20);

			cpu->Reset();

			cycles += cpu->Step();
			cycles += cpu->Step();

			Assert::IsTrue(cpu->Y == 0x53);
			Assert::IsTrue(cpu->X == 0x22);
			Assert::IsTrue(cycles == 6);
		}

		TEST_METHOD(TEST_LDY_ZP)
		{
			uint32_t cycles = 0;
			VM vm(true);
			CPU* cpu = vm.GetCPU();

			vm.WriteData(0xFFFC, 0x00);
			vm.WriteData(0xFFFD, 0x10);

			vm.WriteData(0x10, 0xAB);

			vm.WriteData(0x1000, LDY_ZP);
			vm.WriteData(0x1001, 0x10);

			cpu->Reset();

			cycles = cpu->Step();

			Assert::IsTrue(cpu->Y == 0xAB);
			Assert::IsTrue(cycles == 3);

		}

		TEST_METHOD(TEST_LDY_ZP_X)
		{
			uint32_t cycles = 0;
			VM vm(true);
			uint16_t addr = 0x1000;

			CPU* cpu = vm.GetCPU();

			vm.WriteData(0xFFFC, 0x00);
			vm.WriteData(0xFFFD, 0x10);

			vm.WriteData(0xDD, 0x69);

			vm.WriteData(addr++, LDX_IMM);
			vm.WriteData(addr++, 0xCD);

			vm.WriteData(addr++, LDY_ZP_X);
			vm.WriteData(addr++, 0x10);

			cpu->Reset();

			cycles += cpu->Step();
			cycles += cpu->Step();

			Assert::IsTrue(cpu->Y == 0x69);
			Assert::IsTrue(cpu->X == 0xCD);
			Assert::IsTrue(cycles == 6);
		}

#pragma endregion LDY

	};
}
