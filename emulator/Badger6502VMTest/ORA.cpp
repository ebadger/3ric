...
namespace Badger6502VMTest
{
	...
	public:
		...
		TEST_METHOD(TEST_ORA_ABS)
		{
			...
			vm.WriteData(0x2122, 0x55);
			vm.WriteData(0x22, 0xAA);

			vm.WriteData(addr++, LDA_IMM);
			vm.WriteData(addr++, 0x55);

			vm.WriteData(addr++, ORA_ABS);
		...
		TEST_METHOD(TEST_ORA_ABS_X_CROSSPAGE)
		{
			VM vm(true);
			CPU* cpu = vm.GetCPU();
			uint16_t addr = 0x1000;
		...
		TEST_METHOD(TEST_ORA_ABS_Y_CROSSPAGE)
		{
			VM vm(true);
			CPU* cpu = vm.GetCPU();
			uint16_t addr = 0x1000;
		...
		TEST_METHOD(TEST_ORA_I_ZP_Y_CROSSPAGE)
		{
			VM vm(true);
			CPU* cpu = vm.GetCPU();
			uint16_t addr = 0x1000;
