...
$9000-$BFFF  BASIC ROM overlay, or 12 KiB RAM when $C007 disables the overlay
$C006/$C007  select BASIC ROM / RAM (lower RAM can total 48 KiB)
...
$C800-$CFFF  RAM2
$D000-$FFFF  ROM / monitor / OS / DOS shell, or language-card RAM via $C080-$C08F
```

Language-card RAM has two alternate 4 KiB banks at `$D000-$DFFF` and a shared 8 KiB
at `$E000-$FFFF`. Read selection and write enabling are distinct. Hiding upper ROM also
hides input/output code and interrupt handlers; `SEI` does not mask NMI. `$C800-$CFFF`
contains system state. Browser `loadData` fills backing memory directly and does not prove
that the physical ROM loader populated the selected bank.

The versioned 3RIC Talks brief and bounded submission checker are specified in
[`specs/CODEGEN.md`](../specs/CODEGEN.md) and `challenges/tts/v1/`. They use the existing
assembler and VM without changing the generic harness's halt semantics.
...
| DOS shell | `$EDBC` (`dos`) | Mounts the SD (`fat32_start`) and shows the `>` prompt |
| DOS shell verbs | `cmd_bsave` `$ACFD`, `cmd_bload` `$EF54`, `cmd_brun` `$EF5A`, `cmd_dir` `$D39D`, `cmd_cat` `$D362`, `cmd_chdir` `$EE64`, `cmd_del` `$EE88` | BSAVE/BLOAD/BRUN/DIR/CAT/CD/DEL |
| FAT32 I/O | `fat32_file_write` `$EBB5`, `fat32_file_read` `$EB57` | FAT32 read/write primitives |
